...
def tabulate_tensor_A_coeff(A_, w_, coords_, cell_orientation):
    A = carray(A_, (3, 3), dtype=np.float64)
    w = carray(w_[0], (1), dtype=np.float64)
    coordinate_dofs = carray(coords_, (3, 2), dtype=np.float64)
...
def test_numba_assembly_coeffs():
    mesh = UnitSquareMesh(MPI.comm_world, 13, 13)
    Q = FunctionSpace(mesh, "Lagrange", 1)
    ...
    a = dolfin.cpp.fem.Form([Q._cpp_object, Q._cpp_object], [DG0.element()])
    ...
    fnA = cfunc(sig, cache=True)(tabulate_tensor_A_coeff)
    a.set_cell_tabulate(0, fnA.address)

    # Set a coefficient for the Form
    a.set_coefficient(0, F.cpp_object())
